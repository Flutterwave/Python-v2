...
class Recipient(RaveBase):
    ...
    def _handleCreateResponse(self, response, details):
        responseJson = self._preliminaryResponseChecks(
            response, RecipientCreationError, details["account_number"])
    ...
    def all(self):
        endpoint = self._baseUrl + \
            self._endpointMap["recipient"]["list"] + "?seckey=" + self._getSecretKey()
        return self._handleCardStatusRequests("List", endpoint)

    def fetch(self, recipient_id):
        endpoint = self._baseUrl + self._endpointMap["recipient"]["fetch"] + "/" + str(
            recipient_id) + "?seckey=" + self._getSecretKey()
