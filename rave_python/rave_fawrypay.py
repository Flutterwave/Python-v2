...
class FawryPay(Payment):
    ...
    def charge(self, requestDetails, hasFailed=False):
        ...
        requestDetails.update({"payment_type": "fawry_pay", "currency": "EGP"})
        ...
        requiredParameters = ["amount", "email", "firstname", "lastname", "phonenumber"]

        return super(FawryPay, self).charge(
            requestDetails, requiredParameters, endpoint
        )
    ...
    #     return super(FawryPay, self).verify(txRef, endpoint)

    # def refund(self, flwRef, amount):
    #     endpoint = self._baseUrl + self._endpointMap["account"]["refund"]
    #     return super(FawryPay, self).refund(flwRef, amount)
